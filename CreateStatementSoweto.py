def CreateStatementSoweto():
    create_script = '''CREATE TABLE IF NOT EXISTS soweto_redcap_data(
        study_id text PRIMARY KEY,
        redcap_event_name text,
        --gene_site_id integer, (not in sowero redcap)
        ...
        gene_end_time text, -- to change to time after being able to encoperate NULL values
        gene_compensation integer,
        demo_approx_dob_is_correct integer, -- to include a NULL constraint after incoperating NULL values
        demo_dob_is_correct integer,
        demo_date_of_birth_known integer,
        --demo_dob_new text, -- POPI
        --demo_approx_dob_new text, -- POPI
        -- demo_dob integer text, --POPI
        ...
        --carf_osteo_hip_repl_site integer, not in soweto
        ...
        bppm_measurement_time_1 text, -- to change to TIME datatype
        bppm_systolic_2 integer,
        bppm_diastolic_2 integer,
        bppm_pulse_2 integer,
        bppm_measurement_time_2 text, --to change to TIME datatype
        bppm_systolic_3 integer, 
        bppm_diastolic_3 integer,
        bppm_pulse_3 integer,
        bppm_measurement_time_3 text,-- to change to TIME datatype
        ...
        --comp_end_time text, not in soweto
        completion_of_questionnaire_complete integer
        --genh_oes_cancer_dad integer, not in soweto
        --genh_starchy_staple_food___13 integer, not in soweto
        --genh_starchy_staple_food___14 integer, not in soweto
        --genh_starchy_staple_food___15 integer, not in soweto
        --genh_starchy_staple_food___16 integer, not in soweto
        --carf_diabetes_treat___6 integer, not in soweto
        --bloc_two_purple_tube integer not in soweto
